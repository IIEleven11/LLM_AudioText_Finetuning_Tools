...
for sub_file in tqdm(srt_list):  # Iterate over all srt files
    ...
    max_allowed_gap = (
        1.5 * buffer_time
    )  # maximum gap between subtitles that the script will tolerate without adding additional buffer time.
    ...
    with open("./trainingdata/output.txt", "a+") as out_file:
        ...
        for i, sub in enumerate(subs):
            ...
            if i < len(subs) - 1:
                ...
                next_sub = subs[i + 1]
                next_start_time = (
                    next_sub.start.minutes * 60 + next_sub.start.seconds
                ) * 1000 + next_sub.start.milliseconds
                ...
                if gap_to_next > max_allowed_gap:
                    ...
                    print(
                        f"Added buffer time to segment {i}: New end_time = {end_time}"
                    )
                else:
                    ...
                    end_time += adjustment
                    print(
                        f"Adjusted end_time for segment {i} by {adjustment}ms due to small gap: New end_time = {end_time}"
                    )
            else:
                # If this is the last subtitle, add the buffer time to the end time
                end_time += buffer_time
                print(
                    f"Added buffer time to the last segment {i}: New end_time = {end_time}"
                )
            ...
            if 1050 <= duration <= 11600:  # Adjust these values to your desired range

                audio_segment.export(os.path.join(output_dir, filename), format="wav")
                out_file.write(f"{filename}|{sub.text}|1\n")
            else:
                audio_segment.export(
                    os.path.join(bad_audio_dir, filename), format="wav"
                )
